...
<body>
 ...
 -->
<div class="mt-5">
    <h3 class="display-4">Reporte reductores en mal estado</h3>
</div>
 <div class="content">
 ...
 <div class="card" style="width: 50rem;">
      ...
      <button class="btn btn-success " id="downloadExcel">Descargar</button>
        ...
        <?php if($tipo_report=='danio'){ ?>
        <script>
          var ctx = document.getElementById("chartis").getContext("2d");
          var mychart = new Chart(ctx, {
            type: "<?= $tipo_dia ?>",
            data: {
              labels: ['<?=  $nombres['nombre_1'] ?>', '<?= $nombres['nombre_2'] ?>', '<?= $nombres['nombre_3'] ?>'],
              datasets: [{
    ...
    const dataValues = [<?= $dato1 ?>, <?= $dato2 ?>, <?=$dato3 ?>];

    const labels = ['<?=  $nombres['nombre_1'] ?>', '<?= $nombres['nombre_2'] ?>', '<?= $nombres['nombre_3'] ?>'];
        
        // Obtener el gráfico como imagen
        const canvas = document.getElementById("solicitudes");
        ...
        worksheet.addRow(["Solicitudes", "Valor"]); // Encabezados
